...
uint256 hashGenesisBlock("0x000009585ea181413133cf2d4ca64d2073165876bbc693ef5ae80657d09f34dd");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SWAPCOIN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SWAPCOIN Signed Message:\n";
...
{
    // SWAPCOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    else if(nHeight<=345601)
    {
        nSubsidy = 25 *COIN;
    }
    else if(nHeight<=691201)
    {
        nSubsidy = 12.5 *COIN;
    }
    else if(nHeight<=1382401)
    {
        nSubsidy = 6.25 *COIN;
    }
...
static const int64 nTargetTimespan = 1 * 120; // 60 second diff Retargget
static const int64 nTargetSpacing = 1 * 60; // 60 Second blocks
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SWAPCOIN: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, swapcoin - DarkGravity v2, written by Evan Duffield - evan@swapcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, swapcoin - DarkGravity v3, written by Evan Duffield - evan@swapcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // SWAPCOIN: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // SWAPCOIN: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        hashGenesisBlock = uint256("0x000009ce5a6591e5d49a0b2210ac990ee28decd087684f8d7c7f588ab6d9c908");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Swapcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1410291384;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2137640;
        ...
        assert(block.hashMerkleRoot == uint256("0x93c468af12386f54d5b617933455f1586d8953c991eea95d4d14758f6ae55465"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SWAPCOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SWAPCOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SWAPCOINMiner(CWallet *pwallet)
{
    printf("SWAPCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("swapcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SWAPCOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SWAPCOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SWAPCOINMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - SWAPCOIN **********
    Copyright 2014, Swapcoin Developers 
        eduffield - evan@swapcoin.io
        InternetApe - kyle@swapcoin.io
*/
